"""2026-07-29 全 7 场 — Dixon-Coles fit from de-juiced fair (禁嘴算).
...
TICAI_LINE = {
    "周三001": -1, "周三002": -1, "周三003": -1, "周三004": 1,
    "周三005": -1, "周三006": 1,
}
MATCH = {
    "周三001": ("M-2026-07-29-阿拉木图-奥莫尼亚", "阿拉木图 vs 奥莫尼亚"),
    "周三002": ("M-2026-07-29-波兹南-奥胡斯", "波兹南 vs 奥胡斯"),
    "周三003": ("M-2026-07-29-米拉索尔-里莫", "米拉索尔 vs 里莫"),
    "周三004": ("M-2026-07-29-巴西国际-弗拉门戈", "巴西国际 vs 弗拉门戈"),
    "周三005": ("M-2026-07-29-弗鲁米嫩-巴伊亚", "弗鲁米嫩塞 vs 巴伊亚"),
    "周三006": ("M-2026-07-29-维多利亚-帕梅拉斯", "维多利亚 vs 帕尔梅拉斯"),
}
...
def main():
    ...
    for raw in SNAPS.read_text().splitlines():
        ...
        for num, (mid, _) in MATCH.items():
            if s.get("match_id") == mid and s.get("source") == "sporttery" and "2026-07-29" in s.get("taken_at", ""):
                fairs[num] = s["fair"]
    report = {}
    ...
    out = ROOT / ".nutmeg-data/jczq/daily/2026-07-29/dcfit.json"
