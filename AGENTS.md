...
## JCZQ Daily Decision SOP — 单一入口 `jczq-today`（2026-06-04 重写）

> **本节 2026-06-04 重写**（见 `docs/jczq-decision-chain-critique.md` + spec §32）。
> 历史根因：每个 agent（GPT/Claude）即兴在多套命令/引擎/Poisson 间挑选 → 同一任务
> 决策路径不一致（"每次跳出系统、隔离决策"）。现在**只有一条路径**。
>
> ⚠️ **下方从「#### Step 1」一直到本文件结尾的所有 Step 与「Hard rules / Rules A-J /
> R17-R25 / F2…」整块，描述的是已退役的 Poisson generator（`jczq-daily-brief` +
> `jczq-daily-advisor`）。live 引擎是 `jczq-tiered`，规则见 spec §25-§31。那些内容仅
> 作历史参考，做每日决策时一律忽略——只跑下面这一条 SOP。**
...
> ⚠️ **CLAUDE.md 必须与本节保持一致**（同一份行为指令的两个 harness 副本：Claude 读
> CLAUDE.md，GPT/Codex 读 AGENTS.md）。改一处即同步另一处。

---

<!-- ⬇⬇⬇ 以下全部为【已退役 generator 路径】历史参考，做每日决策时忽略（见上方 SOP）。源码归档见 spec §33。 ⬇⬇⬇ -->

### （DEPRECATED · 退役 generator 历史参考）原 SOP 步骤
...
### （DEPRECATED · 退役 generator）Hard rules (Rules A-J encoded in the generator since 2026-05-05/06)

> ⚠️ 整块 Rules A-J / R17-R25 / F2… 描述的是**退役 Poisson generator** 的选腿逻辑。
> live 引擎 `jczq-tiered` 用 spec §25-§31，**不**用这些。仅作历史参考，勿据此做每日决策。
