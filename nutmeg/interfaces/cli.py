...
from nutmeg.services.content import (
    ...
    DeterministicContentLlmProvider,
    OpenClawContentLlmProvider,
)
...
def build_content_publisher_service(
    *,
    llm_mode: str = "openclaw",
    openclaw_model: str = "nyu-openai-chat/gpt-5.5",
) -> ContentPublisherService:
    mode = llm_mode.strip().casefold()
    if mode == "openclaw":
        provider = OpenClawContentLlmProvider(model=openclaw_model)
    elif mode == "deterministic":
        provider = DeterministicContentLlmProvider()
    else:
...
def build_daily_content_service(*, provider: str = "live") -> DailyContentService:
    ...
    if provider_key == "live":
        ...
        raise ContentValidationError("provider must be `live` or `sample`.")
    return DailyContentService(jczq_provider=calculator_provider)
...
def content_pack(
    ...
    limit: int = CONTENT_LIMIT_OPTION,
    output_dir: Path = CONTENT_OUTPUT_DIR_OPTION,
    llm_mode: str = CONTENT_LLM_MODE_OPTION,
    openclaw_model: str = CONTENT_OPENCLAW_MODEL_OPTION,
    format: str = typer.Option("text", "--format", help="text or json"),
) -> None:
    try:
        service = build_content_publisher_service(
            llm_mode=llm_mode,
    ...
    console.print(
        f"content-pack source={batch.source_report_path} "
        f"packs={len(batch.packs)} llm={batch.llm_provider}"
    )
    if batch.artifacts.json_path:
        console.print(f"json={batch.artifacts.json_path}")
    if batch.artifacts.markdown_path:
        console.print(f"markdown={batch.artifacts.markdown_path}")
    for pack in batch.packs:
        console.print(
            f" - {pack.match_name} risk={pack.risk_level} "
            f"recommendation={pack.publish_recommendation} titles={len(pack.titles)}"
        )
...
def daily_content_pack(
    ...
    if run.artifacts.pdf_path:
        console.print(f"pdf={run.artifacts.pdf_path}")
...
def seedance_submit(
    manifest: Path | None = SEEDANCE_MANIFEST_OPTION,
    run_dir: Path | None = SEEDANCE_RUN_DIR_OPTION,
    confirm: bool = typer.Option(False, "--confirm"),
    match_id: str | None = typer.Option(None, "--match-id"),
    task_key: str | None = typer.Option(None, "--task-key"),
    ratio_key: str = SEEDANCE_RATIO_KEY_OPTION,
    max_concurrency: int = SEEDANCE_MAX_CONCURRENCY_OPTION,
    format: str = typer.Option("text", "--format", help="text or json"),
) -> None:
    try:
        result = build_seedance_service().submit_manifest(
    ...
    except (SeedanceValidationError, SeedanceProviderError) as exc:
        console.print(str(exc))
        raise typer.Exit(code=2) from exc
    ...
    if format == "json":
        ...
        return
    console.print(
        f"seedance-submit submitted={result['submitted']} skipped={result['skipped']} "
        f"manifest={result['manifest_path']}"
    )


@app.command("seedance-poll")
def seedance_poll(
    manifest: Path | None = SEEDANCE_MANIFEST_OPTION,
    run_dir: Path | None = SEEDANCE_RUN_DIR_OPTION,
    download: bool = typer.Option(False, "--download"),
    concat: bool = typer.Option(False, "--concat"),
    ratio_key: str = SEEDANCE_RATIO_KEY_OPTION,
    output_dir: Path | None = SEEDANCE_OUTPUT_DIR_OPTION,
    format: str = typer.Option("text", "--format", help="text or json"),
) -> None:
    try:
        result = build_seedance_service().poll_manifest(
            manifest_path=manifest,
            run_dir=run_dir,
            download=download,
            output_dir=output_dir,
    ...
    console.print(
        "seedance-poll "
        f"succeeded={result['succeeded']} running={result['running']} "
        f"queued={result['queued']} failed={result['failed']} "
        f"concatenated={result.get('concatenated', 0)}"
    )


def _resolve_daily_content_date(value: str) -> str:
