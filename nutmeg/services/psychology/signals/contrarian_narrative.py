    ...
    def evaluate(self, ctx: SignalContext) -> list[SignalReading]:
        ...
        for fx in ctx.fixtures:
            ...
            if len(items) < MIN_SAMPLE:
                out.append(self._abstain(fixture_id, "insufficient sample")); continue
            ...
            if intensity < INTENSITY_GATE or consensus not in {"home", "away"}:
                out.append(self._abstain(fixture_id, f"intensity below gate ({intensity:.2f})")); continue
            contrarian = _opposite(consensus)
            if contrarian is None:
                out.append(self._abstain(fixture_id, "no contrarian outcome")); continue
            out.append(SignalReading(self.name, fixture_id, "HHAD", contrarian, min(intensity, CONVICTION_CAP), [f"Public consensus leans {consensus} at intensity {intensity:.2f}"], [str(it.get("link") or it.get("url") or "") for it in items[:5]], None))
        return out
