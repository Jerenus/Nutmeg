...
> **页面按钮＝同一条命令（2026-09-18 起）**：`uv run nutmeg decision-web` → 顶部 SOP 任务栏按期号列出 B0/B1/B2/B3a/B4/B4b/B6/B6b 八步，点一下＝进程内跑本表同一条 CLI，`task_started/task_done/task_failed` 进当日 `workbench.jsonl`。
> ⛔按钮里没有 B3 深研（要 agent）、B5 构票（判断）、B6b 的 ruling（人填）——那是设计，不是缺功能。
