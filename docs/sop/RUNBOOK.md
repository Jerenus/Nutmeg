...
| B9 | **部署门**+出票+入账 | audit PASS 后跑 `uv run nutmeg zucai-deployment-gate --gate-file <gate.json>`（离线回放加 `--official-history-file <history.json>`）；REVIEW/REDUCE 时优先构造丢整场减注版并重跑，结构化候选确无合格减注版才空仓，**只许减不许加**；用户确认后 ledger 追加（口径注明）；**没入账=没打**；同时落 `<issue>-final-tickets.json` 结构化票面与 `<issue>-af-map.json` 身份映射（faces 不再只住散文） |
