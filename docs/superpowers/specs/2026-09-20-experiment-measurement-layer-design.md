...
# 追加 · T13：从测量回到权重的通路（2026-09-21 立）

## 13.1 目标

用户裁定：**「未来能够更好的识别、迭代评判权重，是更重要的工作。」**

现在缺的不是数据也不是工具，是**通路**：测出来的效应无法变成规则的改动，
所以每条判据的权重仍然是当初拍下来的那个，从未被它自己的战绩修正过。

## 13.2 现状盘点

| | 数量 |
|---|---|
| RULEBOOK 在跑的判据（audit 码） | **19 条**（C0-C18b） |
| `noise_floor` 认识的因子 | **3 个**（c7 / anchor_integrity / death_proof_count） |
| 有前瞻实验在测的 | F1c/F2/F3/F4/F5/F8/F9 共 7 条，**0 条结账** |
| 有零散战绩但没过地板的 | C9（连续证伪）、C11（n=18）、C12（n=14）、C14（2/2）、shield 方向（0/8）等 |

⇒ **十六条判据在指挥出票，却从未被自己的战绩检验过。**

## 13.3 设计

### ① 因子注册表：每条在跑的判据都要有一个因子

新增 `experiments/factors.json`，每条判据一行：

```json
{
  "factor_id": "c7_live_precedent",
  "rule_code": "C7",
  "layer": "fact | probability",          ← 关键三分，见下
  "current_weight_pp": 0.0,
  "weight_band": [0.0, 0.0],
  "row_builder": "experiments.noise_floor:_c7_rows",
  "registered_at": "...",
  "last_measured": {...}
}
```
...
| layer | 含义 | 能不能被测量改 |
|---|---|---|
| `fact` | 宪法第一序事实层否决位（如 C7 改写后的用法、C3 弃模态） | ⛔**不能**。事实层不由价格残差背书，测量只记录不驱动 |
| `probability` | 声称能预测赛果的判据（如 C11/C12 价格带） | ✅ 能。这是本条要迭代的那些 |
| `expression` | 只约束表达形式、不声称预测（如 C17 一致性、C6 降格） | ⛔不适用，不进因子表 |

⇒ **先分层再测量。** 把 `fact` 层的东西拿去做概率检验，就是 2026-09-18 C7 犯过的错；
把 `probability` 层的东西免检，就是现在十六条的状态。

### ② 权重变更规则（**必须在数据到齐之前注册**）

对 `layer == "probability"` 的因子，注册这条规则并冻结：

```
eligible_to_change  ⇔  n >= n_min
                    ∧  verdict == "above_floor"（N0，严格档）
                    ∧  分窗符号一致（训练窗与测试窗同号）

若 eligible：proposed_weight_pp = 点估计，截断进 weight_band
若 n >= n_min 且 verdict == "indistinguishable"：proposed_weight_pp = 0
   —— 这是证伪路径，**权重归零本身就是一次成功的迭代**
其余：维持，不动
```

⛔**先注册规则，再看数据。** 否则数据到手时我会为想要的结论挑判据——
C7 的 +12.0pp → −0.7pp 分窗塌方，正是事后才发现的。

### ③ 人是唯一的执行者
`nutmeg rsi weights propose --factor <id>` 产出**提案**（含 n / 效应 / 地板 / 分窗 / 建议新权重）。
⛔它**不写权重**。改权重只能由人跑 `rsi deploy --by <人>`，与既有治理一致。

### ④ 不做的事
- ⛔不改任何现有权重（本条只建通路，不动一个数）
- ⛔不给 `fact` 层因子做概率检验并据此提议改动
- ⛔不因为某条判据 n 不够就把它从因子表里删掉——**n 不够就是它当前的状态，要看得见**

## 13.4 验收

| # | 检验 | 通过标准 |
|---|---|---|
| E1 | 因子表覆盖 | 19 条 audit 码逐条有归属：进因子表，或标 `expression` 并注明理由 |
| E2 | 分层正确 | C7 标 `fact`（2026-09-18 改写后）；C11/C12 标 `probability` |
| E3 | 规则先于数据 | 变更规则写进注册表且冻结，`git log` 时间早于任何 propose 产物 |
| E4 | propose 不写权重 | 跑完 `current_weight_pp` 一个都没变 |
| E5 | 证伪路径可达 | 构造一个 n 够但 indistinguishable 的因子，确认提议为 0 而不是"维持" |
| E6 | 现状如实 | 大多数因子会是"n 不够、维持"——照实输出，不许隐藏 |
