...
> ⛔**本文是实施前的设计原件，其中四处前提已被实跑推翻**（最严重的一处：§3.1 的「按 `match_id` 取并集」——match_id 不是跨泳道共同键）。
> **请读竣工版**：`2026-09-20-experiment-population-expansion-asbuilt.md`。
> 本文保留不改，作为「设计与实跑偏差」的对照样本。
