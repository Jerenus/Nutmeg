...
## §31 机会识别：软热误定价捕捉（决策原则，2026-05-30 落地）

> ⚠️ **2026-05-30 当日 30 样本回测：本节"软热反面有溢价"假设被证伪 —— 见
> §31.6。** 软热反面（平/冷）30 样本命中仅 23.3%、单位 ROI **−0.257**，比跟随
> 正路（53.3% / −0.090）**更差**。反面的高赔不足以补偿命中率暴跌；市场对软热
> 的"正路 vs 反面"维度定价基本有效，**误定价不在这个维度**。本节框架（机会识别
> 的思路）保留为方法论，但**"软热默认打反面"的具体动作已撤回**——按 §30 纪律：
> 假设先 paper-trade、被数据否决就不上真注。下方原始论证保留作记录。

> **框架定性（原始，方向已被 §31.6 否决）**：§31 不是"风险管理"（躲开危险的
> 软热），是"机会识别"——软热是**被市场错误定价的机会**，要主动捕捉其高赔反面。
> 大众的"主队情结/名气偏好"把热门正路赔率压扁、把平/冷推高。**← 这个"反面被
> 推高=有溢价"的推断，被 §31.6 的 30 样本证伪：反面赔率是对的，不是便宜的。**
> 机会识别的**思路**对（找系统性误定价），但软热"正路 vs 反面"不是那个误定价点。

### §31.0 背景 — 数据锚

用户 5/30 提出："明显的几个大热里，更大胆地选平/弱、规避软热正路、只押有逻辑
支撑的硬热，会提升命中率；决策要更大胆，尤其布兰那场（多轮争议最后选了胜平
=正路热门，结果 1:2 输）。"进一步（同日）："不用规避这些软热——如果判断对，
可以**打反面拿更高收益**。"

`scripts/jczq_calibration.py` + 跨期赛果（6 天 62 场已结算）验证了这个直觉：

| 指标 | 数值 |
|---|---|
| 正路低赔热门（结果赔 <2.0）直接赢 | **37.1%**（23/62）|
| 平局或冷门（非正路热门） | **62.9%** |
| 平局率 | 27.4%（17/62），且**成块**：5/27、5/28 各 0 平；5/24、5/25、5/29 各 5 平 |
| hhad 让球腿命中（§30 表） | 23%（7/31）|

**结论**：跟着每个大热押正路 = 长期输面（仅 37%）。布兰是典型"软热陷阱"——
1.66 主热（implied 56%）、三方共识，却主场 1:2 输；它从来不是 lock，是"37%
那一档里看着最像赢的"。**软热的正路被资金压扁（没价值）、反面被推高（有溢价）
—— 软热本身就是逆向变现的机会，不是要回避的麻烦。**

### §31.1 热门三分层（核心分类）

不再把"低赔热门"当一类，按**赔率结构 + 逻辑支撑**分三层：

| 层 | 判据 | 处理 |
|---|---|---|
| **硬热（hard chalk）** | had ≤ ~1.50 **且** §29 sharp-confirmed（欧赔 fair ≥ 0.55）**且** 有逻辑支撑（实力差/赛意/主客场） | 正路做胆 |
| **软热（soft chalk）** | had 1.55–2.10、**或**体彩偏强但欧赔半信（fair < 0.52）、**或**舒服盘/谨慎博弈场标签 | **不做正路胆；默认打反面（平/冷）吃赔率溢价** |
| **真分歧（coinflip）** | vig 高 + 三方接近 | 最便宜方式表达（§17 既有）|

布兰 1.66 / fair 0.51 / 挪超中游对中游 = **软热**，按 §31 不当正路 A 底仓腿；
正确动作是**打它的反面（让平/平）拿高赔**，而非缩回正路"胜平"。

### §31.2 大胆逆向：默认打反面，护栏在"载体"不在"要不要打"

**修正 5/30 早版的过度保守**（早版写"无逻辑支撑→跳过该场"，把软热溢价白白
扔掉）。新口径：

- **软热默认就打反面**（平 / 冷的一侧），因为反面赔率含市场对热门的系统性高估
  溢价（§31.0 的 37% vs 63% 正是这块溢价的来源）。**不需要额外理由才逆向；
  需要理由的是"做正路"（那要求硬热级支撑）。**
- **方向偏好**（反面押平还是押冷，用盘口选更便宜的一侧）：
  - 客 % > 平 % 或软热方有可证伪弱点（客场无力/轮换/赛意低）→ 偏**冷**
  - 否则（平 % ≥ 客 %、主客实力接近）→ 偏**平**
  - 体彩 implied 显著高于欧赔 fair（资金推高的假热）→ 反面溢价更厚，加重

**真正的护栏 = 载体，不是要不要打**（这是和 D 档盲反 18% 的根本区别）：

1. **分散小注，不串大票**：软热反面用**单场小注 / 2–3 串低串**表达，每注独立
   吃"63% 非正路"的基础概率 + 高赔。**禁止把 N 个软热反面全串一张**——那是
   彩票（整票命中率极低，溢价再高也是负期望放大，= 昨天 E 票 1964 倍的坑）。
2. **总敞口封顶**：软热反面是高方差打法，单日总注金按娱乐预算小额；相关场次
   （同联赛同夜）算作一个敞口，不靠"多注"假装分散。
3. **coinflip 不在此列**：三方接近的场没有可吃的溢价（反面也不便宜），跳过或
   §17 最便宜表达。

D 档盲反反的是"大众站哪边"、还串成大票；§31 反的是"被资金压扁的软热"、用
分散小注吃溢价 —— 同样"反"，载体与依据完全不同。

### §31.3 表述与决策口径

- 把每个大热**显式标层**：硬热 / 软热 / coinflip（接 §30.3 概率口径）。
- **多轮争议后默认偏更大胆一侧**（软热打反面），不要缩回正路 —— 布兰教训：
  来回讨论最后回到正路热门，是"决策疲劳回归大众"，不是真稳。
- **单日结果不证伪本原则**（§30）：软热反面当天若正路赢了，不撤 §31——看 37%
  vs 63% 的长期面 + 反面的赔率溢价，不看单次。
- 守 [[jczq_advice_style]]：不默认"完全不打 / 跳过"，给"有溢价的大胆落点"。

### §31.4 验收 / 追踪（不立即改选腿代码）

- §31 先作**决策原则**（人工 finalize + debate 用），不改 A/B/D/E strategy 代码
  （守 §30 反 churn：原则先跑、攒样本，再考虑代码化）。
- `scripts/jczq_calibration.py` 加追踪维度（backlog）：软热**正路** vs 软热
  **反面**的分层命中率与**实际回报率**（反面赔率高，命中率低但回报可能更优——
  要追的是回报率不只是命中率），样本 ≥30 再决定是否硬编码。
- 立即生效：finalize / debate 时按 §31.1 给每个大热标层；**软热不进正路 A 底仓，
  默认给反面（分散小注）表达**。

### §31.5 不在范围内

- 不改 A/B/D/E strategy 代码（原则层，非代码层）
- 不与 §30 冲突：§31 是"如何更大胆地选"，§30 是"如何用样本验证"——§31 的回报
  提升假设，必须用 §30 校准日志验证，不靠单日结果
- 不退回机械反热门 / 也不退回过度保守的"跳过"（§31.2 载体护栏取代两者）
- 不动信号层 / boldness / chaos

### §31.6 证伪：软热反面 30 样本回测（2026-05-30，本节是 §31 的实证结论）

用户要求"记录软热反面实际回报率、积累样本供未来决策"。`scripts/jczq_softchalk_track.py`
回填 5/24–5/29 全部软热场（fav_odds ∈ (1.50, 2.10]），每场一条反面样本（按
§31.2 方向：冷 implied ≥ 平 implied 押冷，否则押平），对照实际 had：

| 打法 | n | 命中率 | 单位 ROI |
|---|---|---|---|
| **软热反面**（平/冷） | 30 | **23.3%** | **−0.257** |
| 跟随软热正路 | 30 | 53.3% | −0.090 |

**结论：假设证伪。** 软热反面命中仅 23.3%（30 场中 7 中），反面的高赔（均 ~3.3）
**不足以补偿命中率暴跌**，单位 ROI −25.7% 比跟随正路（−9.0%）还差 ~17pp。

**为什么 §31.0 的"37% vs 63%"具误导性**：63% 是"平 OR 冷"的合计，但下注**只能
选一个反面方向**——单方向反面只有 23%。市场对软热的正路/反面定价**基本有效**，
误定价不在这个维度。"反面赔率高"是因为它**该这么高**（对应低命中率），不是便宜。

**动作（守 §30 纪律）**：
- **撤回"软热默认打反面"**——paper-trade 否决，不上真注。
- 软热回到中性处理：**正路无溢价（−9%）、反面更差（−26%）→ 软热场本质是抽水
  陷阱，最优是少碰 / 极小注 / 只在有独立基本面理由时单点表达**，而非系统性
  反向。
- `softchalk-track.json` 继续累积（每日复盘自动并入），样本到 60/90 再复核
  结论是否稳定；若某子类（如特定联赛 / 特定赔率带）反面 ROI 转正，再单独提取。
- **方法论价值保留**：这次"逻辑上很顺的假设 + 30 样本一跑就被否"正是 §30 存在
  的意义——**先 paper-trade 再下注**，省下了按错误逻辑实押的钱。

### §31.7 真正的下一步（机会识别的正确方向）

软热"正路 vs 反面"维度无 edge，不代表没有误定价；只是不在这个维度。后续按 §30
用 paper-trade 验证（不凭逻辑直接上）的候选维度：
- **§29 体彩−欧赔 gap**：体彩 implied 显著高于欧赔 fair 的"假热"，反它是否有
  edge（这是有外部基准的误定价，比"软热反面"更可能真实）
- **§28 R25 联赛进球偏差**：特定联赛 ttg 系统性偏差（已有数据基础）
- 每个候选都先进 paper-trade track，ROI 转正且 n≥30 才考虑真注 / 代码化。
