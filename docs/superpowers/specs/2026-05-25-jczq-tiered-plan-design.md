...
用户 5/30 提出："明显的几个大热里，更大胆地选平/弱、规避软热，只押有逻辑
...
1.66 主热（implied 56%）、GPT+Claude+足彩盘三方共识，却主场 1:2 输；它从来
不是 lock，是"37% 那一档里看着最像赢的"。
...
| 层 | 判据 | 处理 |
|---|---|---|
| **硬热（hard chalk）** | had ≤ ~1.50 **且** §29 sharp-confirmed（欧赔 fair ≥ 0.55，欧赔同认）**且** 有逻辑支撑（实力差/赛意/主客场） | 可做胆、正路表达 |
| **软热（soft chalk）** | had 1.55–2.10、**或**体彩偏强但欧赔半信（fair < 0.52）、**或**舒服盘/谨慎博弈场标签 | **规避正路**：宁可逆向选平/弱，或整场不碰 |
| **真分歧（coinflip）** | vig 高 + 三方接近 | 最便宜方式表达（§17 既有）|

布兰 1.66 / fair 0.51 / 挪超中游对中游 = **软热**，按 §31 应规避正路（选让平/平
或跳过），而非当 A 底仓腿。

### §31.2 大胆逆向的护栏（防止退化成"机械反热门"）

用户要"大胆、创造力、人性逆向"，但 D 档历史命中仅 18% 证明**无脑反热门也是坑**。
逆向必须**有逻辑支撑**，三选一才允许逆向：

1. **盘口逻辑**：体彩 implied 显著高于欧赔 fair（软热被资金推高，§29 反向）
2. **基本面逻辑**：软热一方有可证伪的弱点（客场无力/轮换/赛意低/伤停）
3. **结构逻辑**：软热扎堆日（≥3 场软热）→ 平局/冷门成块概率升（§31.0 数据）

**无任何一条支撑 → 不逆向**（回到"软热就跳过该场"，而非硬押反面）。这区别
于 D 档的盲反：D 反的是"大众站哪边"，§31 反的是"有具体逻辑的软热"。

### §31.3 表述与决策口径

- 把每个大热**显式标层**：硬热 / 软热 / coinflip（接 §30.3 概率口径）。
- **多轮争议后默认偏向更大胆的一侧**（选平/弱/逆向），只要 §31.2 有一条支撑——
  布兰教训：来回讨论最后缩回正路热门，是"决策疲劳回归大众"，不是真稳。
- 但**单日结果不证伪原则**（§30）：逆向选平当天若热门正路赢了，不撤 §31——
  看的是 37% vs 63% 的长期面。
- 仍守 [[jczq_advice_style]]：不默认"完全不打"，给"有逻辑的大胆落点"。

### §31.4 验收 / 追踪（不立即改选腿代码）

- §31 先作**决策原则**（人工 finalize + debate 用），不改 A/B/D/E strategy 代码
  （守 §30 反 churn：原则先跑、攒样本，再考虑代码化）。
- `scripts/jczq_calibration.py` 加追踪维度（backlog）：软热正路 vs 软热逆向的
  分层命中率，样本 ≥30 再决定是否硬编码进 A 档候选。
- 唯一立即生效：**finalize / debate 时按 §31.1 给每个大热标层，软热不默认进
  A 底仓正路腿**（布兰类不再当稳腿）。

### §31.5 不在范围内

- 不改 A/B/D/E strategy 代码（原则层，非代码层）
- 不与 §30 冲突：§31 是"如何更大胆地选"，§30 是"如何用样本验证"——§31 的命中
