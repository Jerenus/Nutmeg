...
- **分歧**（§35.6，**警示非方向**）：~29 家书商 implied 分歧。⚠️ 绝对阈值会标 11/15=噪声
  （05-30 实测），改用**相对 top-2**：均值 ≥ max(`DISPERSION_FLOOR=0.13`, 全场中位数×
  `1.30`) 且取最软的 ≤2 场。pick="谨慎·高方差"——"这场别把热门当稳腿"，与 chaos 值部分重叠。
