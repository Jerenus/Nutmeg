...
- **分歧**（§35.6，**警示非方向**）：书商对**热门那一路**定价的分歧。两次实测迭代：
  ① 05-30 绝对阈值标 11/15=噪声 → 改相对 top-2；② 06-07 绝对离散在哥伦比亚 1.15 报 71%
  =算法假象（赔率越短被放大）→ 改用**热门路 implied 变异系数 CV**（尺度无关）：CV ≥
  max(`DISPERSION_CV_FLOOR=0.06`, 全场中位×`1.30`) 取最软 ≤2 场。pick="谨慎·热门价软"。
...
21 测试 `tests/test_jczq_opportunity.py`。非 edge/非概率、纯确定性、无 LLM、无 I/O。
...
### §35.7 收敛（2026-06-07）—— 5 透镜封顶，反 churn
> 实测反思后**主动收敛**，不重蹈 R1-R28 churn。各透镜实证价值排序（05-30 真实盘）：
> - **大小球** 5/15 强中、有方向 = 最实用；**反面** 6/15（混乱盘多、正常盘锐）= 旗舰；
>   **异动** 0（存档无异动、live 有值）= 条件性。这三个是好轨道。
> - **平局** 0/15（罕触发但干净）= 边际保留；**分歧** 修前 11/15 噪声→相对 top-2 后 0-2 = 警示层。
>
> **决定：雷达收敛在 5 透镜，停止再加。** 未来加透镜的**门槛抬高**：必须先在真实/回测盘上
> 验证触发率与锐度（不糊、不哑），再注册——不是"可计算"就加。流程教训：**新透镜先跑真盘
> 看 fire-rate，再 commit**（分歧若早跑真盘就不会先 ship 噪声）。

### §35.8 06-07 复盘驱动的迭代（修 bug，**不** churn）
> 2026-06-07 4 友谊赛复盘。**迭代什么 vs 不迭代什么**，守 §35.7 纪律：
> - ✅ **修**：分歧透镜绝对离散 → 热门路 CV（哥伦比亚 1.15 假 71%→真 17%）。**确定性 bug**、
>   赛前已独立预判、赛后证实=误报，不是单日 churn。见 §35.6。
> - ❌ **不动 大小球**：06-07 走 0/2（201 喊小球实际 over、204 喊大球实际欠）——**单日 2 腿=噪声**，
>   §35.7/§30 纪律：单日不杀透镜。继续观察。
> - ❌ **不动 A 选腿**：A 让球-1 cover 又败（克罗地亚赢 1 球=让平），这是 06-04/06-06/06-07 **重复
>   模式**（低进球→小胜→cover 失）。机制清晰、且大小球信号独立印证——但**改 A 选腿=churn**，
>   §32.6/§35.7 禁。**只 track，不改**；若回测样本（非 3 个轶事）证实，再考虑原则性改动。
> - 自评：我 §C 答"希腊胜"错（意大利赢）；但赛前预判"克罗地亚-1 是软肋"命中。友谊赛热门双向不可靠。
