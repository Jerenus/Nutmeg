...
def test_emits_reading_when_snapshot_present() -> None:
    ...
    sig = ReflexiveTacticSignal(llm=FakeLLMCompleter(responses=['{"second_order_pick":"away_win","conviction":0.7,"reasoning":["Kompany inverts to compact 5"]}']))
...
def test_missing_snapshot_abstains() -> None:
    ...
    [r] = sig.evaluate(SignalContext(date="2026-04-29", fixtures=[{"id": "f1", "home_team_name": "PSG", "away_team_name": "Bayern"}], snapshots={}, odds={}))
...
def test_llm_failure_abstains() -> None:
    ...
    [r] = sig.evaluate(SignalContext(date="2026-04-29", fixtures=[{"id": "f1", "home_team_name": "PSG", "away_team_name": "Bayern"}], snapshots={"f1": {"home_shape": "x", "away_shape": "y"}}, odds={}))
    assert r.outcome_view is None
    assert "parse" in (r.abstain_reason or "")

def test_invalid_pick_abstains() -> None:
    sig = ReflexiveTacticSignal(llm=FakeLLMCompleter(responses=['{"second_order_pick":"weird","conviction":0.9,"reasoning":[]}']))
    [r] = sig.evaluate(SignalContext(date="2026-04-29", fixtures=[{"id": "f1", "home_team_name": "PSG", "away_team_name": "Bayern"}], snapshots={"f1": {"home_shape": "x", "away_shape": "y"}}, odds={}))
    assert r.outcome_view is None
