...
def test_parser_uses_llm_when_available() -> None:
    parser = InspirationParser(llm=FakeLLMCompleter(responses=['{"lean":"psychology","conviction":"high","focus":["tournament_stage"],"force_psychology":false,"force_data":false}']))
...
def test_parser_regex_fallback_when_llm_fails() -> None:
    note = InspirationParser(llm=FakeLLMCompleter(responses=["not json"])).parse("反着来，淘汰赛风险大", date="2026-04-29")
    assert note.parsed_tags.lean == "psychology"
...
def test_regex_detects_force_flags() -> None:
    note = InspirationParser(llm=FakeLLMCompleter(responses=["bad"])).parse("今天必须心理，强制心理层", date="2026-04-29")
    assert note.parsed_tags.force_psychology is True

def test_default_neutral_when_unparseable_text() -> None:
    note = InspirationParser(llm=FakeLLMCompleter(responses=["bad"])).parse("Nothing relevant.", date="2026-04-29")
