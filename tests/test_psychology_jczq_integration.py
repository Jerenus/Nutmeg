...
class _StubProvider:
    ...
    def evaluate(self, ctx: SignalContext) -> list[SignalReading]:
        ...
        for fx in ctx.fixtures:
            ...
            if target:
                readings.append(SignalReading(self.name, str(fx.get("id")), "HHAD", target, 0.85, ["forced for test"], [], None))
        return readings

def test_disabled_psychology_layer_keeps_data_picks() -> None:
    report = JczqMixedReportService(provider=SampleJczqCalculatorProvider()).build_report(dry_run=True)
...
def test_enabled_layer_overrides_one_leg(monkeypatch) -> None:
    ...
    svc = JczqMixedReportService(provider=sample_provider, psychology_engine=PsychologyEngine(providers=[_StubProvider({target_fixture_id: flip_target})]), reconciliator=Reconciliator(ConvictionGate(0.7), BudgetGuard(1)))
