...
def test_content_pack_command_generates_json_artifacts(tmp_path) -> None:
    result = runner.invoke(
        app,
        [
            "content-pack",
            "--report-file",
            "nutmeg/content/samples/26068-content-report.json",
            "--limit",
            "1",
            "--output-dir",
    ...
    assert len(payload["packs"][0]["titles"]) == 5
    assert payload["packs"][0]["short_video_script"].endswith(
        "以上只是赛前数据观察，不构成任何投注建议，理性看球。"
    )
    assert payload["packs"][0]["long_article"].endswith(
        "本文仅基于公开信息进行足球赛事数据分析和规则科普，不构成任何投注建议，也不引导任何形式的下注。比赛结果具有不确定性，请理性看待。"
    )
    assert payload["packs"][0]["compliance_checklist"]
    assert payload["packs"][0]["publish_recommendation"] in {"publish", "review", "skip"}
    assert Path(payload["artifacts"]["json_path"]).exists()
    assert Path(payload["artifacts"]["markdown_path"]).exists()


def test_content_pack_command_rejects_missing_report(tmp_path) -> None:
    result = runner.invoke(
        app,
        [
            "content-pack",
...
def test_zucai_odds_sync_command_updates_registry(tmp_path) -> None:
    ...
    registry_file.write_text(
        json.dumps({"entries": [{"issue_id": "26068", "issue_file": "26068-issue.json"}]}),
        encoding="utf-8",
    )
    result = runner.invoke(
        app,
        [
            "zucai-odds-sync",
            "--source-file",
            "nutmeg/zucai/samples/26068-odds-source-afternoon.html",
            "--issue-id",
            "26068",
            "--slot",
            "afternoon",
            "--captured-at",
            "2026-04-26 16:00 CST",
            "--output-dir",
            str(tmp_path),
            "--registry-file",
            str(registry_file),
    ...
    payload = json.loads(result.stdout)
    assert payload["issue_id"] == "26068"
    assert payload["slot"] == "afternoon"
    assert payload["parsed_count"] == 14
    assert Path(payload["odds_path"]).exists()
    entry = json.loads(registry_file.read_text(encoding="utf-8"))["entries"][0]
    assert entry["odds_file"] == "26068-odds.json"


def test_zucai_odds_sync_command_rejects_url_without_live_fetch(tmp_path) -> None:
    result = runner.invoke(
        app,
        [
            "zucai-odds-sync",
            "--source-url",
            "https://example.test/odds.html",
            "--issue-id",
            "26068",
            "--slot",
            "afternoon",
...
def test_jczq_mixed_report_command_generates_pdf_artifacts(tmp_path) -> None:
    result = runner.invoke(
        app,
        [
            "jczq-mixed-report",
    ...
    assert result.exit_code == 0
    payload = json.loads(result.stdout)
    assert payload["official_last_update"] == "2026-04-26 18:33:24"
    assert len(payload["combinations"]) == 2
    assert len(payload["combinations"][0]["legs"]) == 4
...
def test_seedance_submit_command_refuses_without_confirm(tmp_path) -> None:
    ...
    result = runner.invoke(
        app,
        ["seedance-submit", "--manifest", str(manifest), "--format", "json"],
    )
...
def test_seedance_poll_command_accepts_run_dir_and_concat(tmp_path, monkeypatch) -> None:
    manifest = tmp_path / "seedance-manifest.json"
    manifest.write_text(json.dumps({"run_id": "run-1", "tasks": []}), encoding="utf-8")
    calls = []

    class FakeSeedanceService:
        def poll_manifest(self, **kwargs):
            calls.append(kwargs)
            return {
    ...
    monkeypatch.setattr(
        "nutmeg.interfaces.cli.build_seedance_service",
        lambda: FakeSeedanceService(),
    )
