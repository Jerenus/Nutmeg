...
def test_lifecycle_with_inspiration_force_psychology_below_gate(tmp_path: Path) -> None:
    ...
    note = InspirationParser(llm=FakeLLMCompleter(responses=[])).parse(
        "今天必须心理，反着来", date="2026-04-29"
    )
    ...
    rec.update_outcome(
        date="2026-04-29", fixture_id="psg-bay", market="HHAD", actual_outcome="away_win"
    )
