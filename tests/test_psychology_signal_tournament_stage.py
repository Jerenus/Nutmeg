...
def test_first_leg_ucl_knockout_leans_draw_under() -> None:
    fixtures = [{"id": "psg-bay", "competition_code": "UCL", "stage": "knockout", "leg": 1, "tier_home": 1, "tier_away": 1, "aggregate_score_diff": None}]
...
def test_second_leg_aggregate_tied_leans_home_over() -> None:
    fixtures = [{"id": "f2", "competition_code": "UEL", "stage": "knockout", "leg": 2, "tier_home": 1, "tier_away": 1, "aggregate_score_diff": 0}]
    readings = TournamentStageSignal().evaluate(_ctx(fixtures))
...
def test_no_matching_rule_yields_abstain() -> None:
    fixtures = [{"id": "f3", "competition_code": "PL", "stage": "regular", "leg": None, "tier_home": 1, "tier_away": 1, "aggregate_score_diff": None}]
    readings = TournamentStageSignal().evaluate(_ctx(fixtures))
