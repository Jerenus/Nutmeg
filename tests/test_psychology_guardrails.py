...
def _candidate(leg_id: str, conviction: float, provider: str = "tournament_stage") -> OverrideCandidate:
    return OverrideCandidate(leg_id=leg_id, fixture_id="f"+leg_id, market="HHAD", from_outcome="home_win", to_outcome="away_win", conviction=conviction, reasoning=[], source_provider=provider)

def test_conviction_gate_filters_below_threshold() -> None:
    accepted, rejected = ConvictionGate(threshold=0.7).apply([_candidate("1", 0.65), _candidate("2", 0.72)])
    assert [c.leg_id for c in accepted] == ["2"]
    assert rejected[0][1].startswith("below_conviction")

def test_budget_guard_caps_to_one() -> None:
    accepted, rejected = BudgetGuard(max_reversals=1).apply([_candidate("1", 0.8), _candidate("2", 0.9)])
