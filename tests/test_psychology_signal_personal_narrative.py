...
def test_story_present_and_advantage_emits_reading() -> None:
    rss = MagicMock()
    rss.fetch.return_value = [{"title": "Kompany 拜仁欧冠首秀", "link": "u"}]
    sig = PersonalNarrativeSignal(
        rss=rss,
        llm=FakeLLMCompleter(
            responses=[
                '{"story_present":true,"team_advantaged":"away","conviction":0.55,'
                '"story_summary":"Kompany debut narrative"}'
            ]
        ),
    )
...
def test_no_story_abstains() -> None:
    rss = MagicMock()
    rss.fetch.return_value = [{"title": "random", "link": ""}]
    sig = PersonalNarrativeSignal(
        rss=rss,
        llm=FakeLLMCompleter(
            responses=[
...
def test_llm_failure_abstains() -> None:
    rss = MagicMock()
    rss.fetch.return_value = [{"title": "x", "link": ""}]
