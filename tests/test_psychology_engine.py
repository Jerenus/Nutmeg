...
def test_aggregates_agreeing_signals() -> None:
    p1 = MagicMock(); p1.name = "tournament_stage"; p1.evaluate.return_value = [_reading("tournament_stage", "f1", "HHAD", "away_win", 0.65)]
    p2 = MagicMock(); p2.name = "contrarian_narrative"; p2.evaluate.return_value = [_reading("contrarian_narrative", "f1", "HHAD", "away_win", 0.7)]
    [v] = PsychologyEngine(providers=[p1, p2]).evaluate(ctx=SignalContext(date="2026-04-29", fixtures=[{"id":"f1"}], snapshots={}, odds={}), data_picks={"f1": {"HHAD": "home_win"}})
...
def test_disagreeing_signals_pick_highest_conviction() -> None:
    ...
    [v] = PsychologyEngine(providers=[p1, p2]).evaluate(ctx=SignalContext(date="2026-04-29", fixtures=[{"id":"f1"}], snapshots={}, odds={}), data_picks={"f1": {"HHAD": "draw"}})
    assert v.market_views["HHAD"].outcome == "away_win"
...
def test_all_abstain_neutral() -> None:
    ...
    [v] = PsychologyEngine(providers=[p]).evaluate(ctx=SignalContext(date="2026-04-29", fixtures=[{"id":"f1"}], snapshots={}, odds={}), data_picks={"f1": {"HHAD": "draw"}})
...
def test_agreement_with_data_pick() -> None:
    p = MagicMock(); p.name = "p"; p.evaluate.return_value = [_reading("p","f1","HHAD","home_win",0.7)]
    [v] = PsychologyEngine(providers=[p]).evaluate(ctx=SignalContext(date="2026-04-29", fixtures=[{"id":"f1"}], snapshots={}, odds={}), data_picks={"f1": {"HHAD": "home_win"}})
