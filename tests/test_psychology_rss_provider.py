...
def test_fetches_each_feed_and_merges(tmp_path: Path) -> None:
    ...
    provider = RssProvider(feeds=["https://a/feed.xml", "https://b/feed.xml"], fetcher=fake_fetcher, cache=cache)
...
def test_filters_by_query_when_supplied(tmp_path: Path) -> None:
    fake_fetcher = MagicMock(side_effect=[{"entries": [
...
def test_single_feed_failure_does_not_block(tmp_path: Path) -> None:
    fake_fetcher = MagicMock(side_effect=[RuntimeError("404"), {"entries": [{"title": "ok", "link": "https://b/1", "summary": ""}]}])
    cache = NewsCache(base_dir=tmp_path)
