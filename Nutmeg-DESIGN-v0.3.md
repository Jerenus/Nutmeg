# Nutmeg · 过人
**足球评估分析 AI Agent · 项目设计 v0.3**

> v0.3 变更：§3.1、§4.1、§4.2、§4.4.5 的 ASCII 架构图全部升级为 **Mermaid 图**，GitHub/GitLab/VSCode 原生渲染。
...
```mermaid
flowchart TB
    subgraph L3["Layer 3 · 专业事件数据（训练/研究）"]
        SB["StatsBomb 开放数据<br/>免费 · 3000+ 场历史<br/>JSON + 360 冻结帧"]
    end

    subgraph L2["Layer 2 · 实时运营数据（赛程/赔率/基础统计）"]
        APIF["API-Football<br/>$19-39/月 · 960+ 联赛<br/>含实时赔率与预测"]
    end

    subgraph L1["Layer 1 · 深度补充数据（免费爬虫/数据集）"]
        SD["soccerdata<br/>FBref · Understat · Sofascore · FotMob"]
        TM["transfermarkt-datasets<br/>身价 · 伤病 · 转会 · 每周更新"]
        CE["Club Elo<br/>历史 Elo"]
    end

    subgraph Std["标准化层"]
        K["kloppy<br/>坐标/事件/追踪统一"]
        SP["SPADL<br/>事件语义统一"]
    end

    subgraph Store["存储层（Phase 1）"]
        DDB[("DuckDB<br/>分析查询")]
        SL[("SQLite<br/>状态 · user_id 分片")]
        PQ[("Parquet<br/>归档")]
    end
...
**三层的分工本质**：
- **Layer 3** 像"博物馆的高清藏品"——用来训练模型和做历史研究，覆盖有限但质量最高
- **Layer 2** 像"每日报纸"——近实时、覆盖广、质量中等，负责运营类一切
- **Layer 1** 像"爱好者的资料本"——免费但需要维护，补足 L2 的深度指标
...
flowchart TD
    ...
    Web["Web UI · Phase 3 后期"] -.未来.-> Router

    Router{"Router<br/>意图分类"}
    Router --> Tactics["TacticsAgent<br/>战术分析"]
    Router --> Odds["OddsAgent<br/>盘口分析"]
    Router --> Player["PlayerAgent<br/>球员数据"]
    Router --> Synth["SynthesisAgent<br/>综合判断"]

    Synth -.召唤.-> Tactics
    Synth -.召唤.-> Odds
    Synth -.召唤.-> Player

    Tactics --> Tools["Tools 层（MCP-Ready）<br/>DataFetch · Model · Viz · Memory"]
    Odds --> Tools
    Player --> Tools
    Synth --> Tools

    Tools --> DM["数据与模型层<br/>kloppy 标准化 + 模型服务"]
    DM --> DDB[("DuckDB · 分析")]
    DM --> SL[("SQLite · user_id 分片")]

    DDB --> Ext["外部数据源"]
    SL -.偏好/记忆.-> Mem["per-user config"]

    Ext --> APIF["API-Football"]
    Ext --> SD["soccerdata"]
    Ext --> TM["transfermarkt-datasets"]
    Ext --> SBOpen["StatsBomb Open"]

    Tactics -.trace.-> LS["LangSmith<br/>Trace + Evals"]
    Odds -.trace.-> LS
    Player -.trace.-> LS
    Synth -.trace.-> LS
```
...
```mermaid
flowchart LR
    User(["👤 用户查询<br/>'周末曼市德比分析+下注建议'"])

    Router{"Router<br/>意图分类器"}
    Classify{信息型?<br/>判断型?}

    subgraph Parallel["专家 Agents（并行召唤）"]
        direction TB
        T["TacticsAgent<br/>近5场战术画像<br/>预期阵型"]
        O["OddsAgent<br/>Dixon-Coles 模型<br/>市场对比"]
        P["PlayerAgent<br/>核心伤病<br/>近期状态"]
    end

    Direct["直接数据查询<br/>（无需激进决策）"]

    Synth["SynthesisAgent<br/>跨模块综合<br/>+ 主观风险提示"]

    Output["📋 四段式输出<br/>判断 → 核心理由 → 反驳角度 → 置信度"]

    User --> Router --> Classify
    Classify -->|判断型| T
    Classify -->|判断型| O
    Classify -->|判断型| P
    Classify -->|信息型| Direct
...
**关键设计点**：
- 专家 Agent 之间**并行**而非串行，降低延迟
- `Classify` 节点决定是否启用激进决策模式——查数据时保守，做判断时激进
- 所有决策类输出**强制**走四段式模板，由 SynthesisAgent 统一组装
...
```mermaid
flowchart LR
    subgraph P1["Phase 1 · 个人工具（0-6 月）"]
        direction TB
        P1D["本机 / 个人 VPS<br/>单容器"]
        P1U["user_id = 'owner'"]
        P1S["DuckDB + SQLite"]
        P1I["CLI + IM Bot"]
    end

    subgraph P2["Phase 2 · 朋友共享（6-12 月）"]
        direction TB
        P2D["云 VPS · 多容器"]
        P2U["授权码登录<br/>多 user_id"]
        P2S["PostgreSQL + Redis"]
        P2I["+ 管理 CLI"]
    end

    subgraph P3["Phase 3 · 订阅服务（12+ 月）"]
        direction TB
        P3D["云原生 · K8s / Serverless"]
        P3U["正式认证 + Stripe"]
        P3S["托管 DB + 对象存储"]
        P3I["+ Web UI"]
    end

    P1 ==拧存储/加授权==> P2 ==商业化/运维升级==> P3
```
...
| v0.3 | 2026-04-24 | 4 张架构图改为 Mermaid 并嵌入文档：①§3.1 三层数据架构；②§4.1 系统总体架构；③§4.2.0 Agent 编排流程（新增）；④§4.4.5 Phase 演进路径。GitHub/GitLab/VSCode 可原生渲染 |

---

**下一步动作建议：**
1. 在 GitHub 私有仓新建项目，把 DESIGN.md 作为首个 commit——你会立刻看到 Mermaid 图在 GitHub 上的渲染效果
2. 方向 OK 后，可以立即进入 Sprint 0：
   - 我帮你起一份 **Sprint 0 的代码骨架**（`pyproject.toml` + 完整目录 + 最小 CLI + LangSmith 接入 stub + Repository 抽象雏形）
   - 或起一版面向 GitHub README 的项目介绍（中英双语，含 logo 占位、徽章、一句话定位）
3. §9.4 的 3 个新开放问题（激进决策刹车、博彩合规、LangSmith PII）可以在 Sprint 0 期间继续沉淀，不必现在拍板
